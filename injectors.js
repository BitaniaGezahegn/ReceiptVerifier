...
export function showRandomReviewModal(html, mgmtTabId, rowId, extractedId, imgUrl) {
  ...
  export function showDuplicateModal(html, id, amount) {
      const host = document.createElement('div');
      ...
      shadow.getElementById('btn-copy-exit').onclick = (e) => { e.stopPropagation(); navigator.clipboard.writeText(status).then(() => { chrome.runtime.sendMessage({ action: "closeTab" }); done(); }); };
    ...
    return new Promise((resolve) => {
      chrome.runtime.sendMessage({ action: "fetchImageBase64", url: imgSrc }, (response) => {
        ...
        img.onload = () => {
          ...
          resolve({
              processed: canvas.toDataURL('image/jpeg', 1.0).split(',')[1],
              raw: response.data,
